<!DOCTYPE html>
<html>
  <head>
    <!--Import Google Icon Font-->
    <link href="http://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <!--Import materialize.css-->
    <link type="text/css" rel="stylesheet" href="css/materialize.min.css"  media="screen,projection"/>
    <link type="text/css" rel="stylesheet" href="css/editcss.css" />
    <!--Let browser know website is optimized for mobile-->
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <!-- Nav bar -->
    <nav>
    <ul id="slide-out" class="side-nav fixed">
      <li class=""><a href="List.php" class="waves-effect waves-teal"><i class="material-icons">view_stream</i>กำหนดเวลา</a></li>
      <li class=""><a href="Day.php" class="waves-effect waves-teal"><i class="material-icons">view_day</i>วัน</a></li>
      <li class=""><a href="3Day.php" class="waves-effect waves-teal"><i class="material-icons">view_column</i>3 วัน</a></li>
      <li class=""><a href="Week.php" class="waves-effect waves-teal"><i class="material-icons">view_column</i>สัปดาห์</a></li>
      <li class="active"><a href="Month.php" class="waves-effect waves-teal"><i class="material-icons">view_module</i>เดือน</a></li>
      <li class=""><a href="Search.php" class="waves-effect waves-teal"><i class="material-icons">search</i>ค้นหา</a></li>
      <li class="divider"></li>
      <li class=""><a href="" class="waves-effect waves-teal"><i class="material-icons">account_circle</i>chickenjieb@gmail.com</a></li>
      <li class=""><a href="" class="waves-effect waves-teal">กิจกรรม</a></li>
      <li class=""><a href="" class="waves-effect waves-teal">แจ้งเตือน</a></li>
      <li class=""><a href="" class="waves-effect waves-teal"><i class="material-icons">account_circle</i>chickenjieb@gmail.com</a></li>
      <li class=""><a href="" class="waves-effect waves-teal">Samsung Calendar</a></li>
      <li class=""><a href="" class="waves-effect waves-teal"><i class="material-icons">cake</i>เพิ่มเติม</a></li>
      <li class=""><a href="" class="waves-effect waves-teal">วันเกิด</a></li>
      <li class=""><a href="" class="waves-effect waves-teal">วันหยุด</a></li>
      <li class="divider"></li>
      <li class=""><a href="Setting.php" class="waves-effect waves-teal"><i class="material-icons">settings</i>การตั้งค่า</a></li>
      <li class=""><a href="Help.php" class="waves-effect waves-teal"><i class="material-icons">help</i>ความช่วยเหลือและความคิดเห็น</a></li>
      </li>
    </ul>
    <a href="#" data-activates="slide-out" class="button-collapse" hide-on-large-only><i class="material-icons grey-text">menu</i></a>
    <div class="nav-wrapper">
        <a href="#" class="brand-logo left black-text" hide-on-med-and-down>เมษายน</a>
        <ul class="right">
          <li><a href=""><i class="material-icons grey-text">perm_contact_calendar</i></a></li>
          <li><a href=""><i class="material-icons grey-text">more_vert</i></a></li>
        </ul>
      </div>
  </nav>
 
  </head>

  <body style="background-color: #FFFFFF;">
    <table class="bordered outside-table-month">
        <thead class="outside-table-month">
          <tr>
              <th>จ</th>
              <th>อ</th>
              <th>พ</th>
              <th>พฤ</th>
              <th>ศ</th>
              <th>ส</th>
              <th>อา</th>
          </tr>
        </thead>

        <tbody>
          <tr class="new-row-month">
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>27</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>28</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>29</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>30</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>31</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>1</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>2</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
          </tr>
          <tr class="new-row-month">
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>3</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>4</th>
                  </tr>
                </thead>
                <tbody>
                  <td style="background-color: #BFC932;">Brief Project</td>
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>5</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>6</th>
                  </tr>
                </thead>
                <tbody>
                  <td>วันจักรี</td>
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>7</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>8</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>9</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
          </tr>
          <tr class="new-row-month">
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>10</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>11</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>12</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>13</th>
                  </tr>
                </thead>
                <tbody>
                  <td>หยุดชดเชยวันสงกรานต์</td>
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>14</th>
                  </tr>
                </thead>
                <tbody>
                  <td>หยุดชดเชยวันสงกรานต์</td>
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>15</th>
                  </tr>
                </thead>
                <tbody>
                  <td>หยุดชดเชยวันสงกรานต์</td>
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>16</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
          </tr>
          <tr class="new-row-month">
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>17</th>
                  </tr>
                </thead>
                <tbody>
                  <td>หยุดชดเชยวันสงกรานต์</td>
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>18</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>19</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>20</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>21</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>22</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>23</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
          </tr>
          <tr class="new-row-month">
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>24</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>25</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>26</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>27</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>28</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>29</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>30</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
          </tr>
          <tr class="new-row-month">
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>1</th>
                  </tr>
                </thead>
                <tbody>
                  <td>วันแรงงาน</td>
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>2</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>3</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>4</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>5</th>
                  </tr>
                </thead>
                <tbody>
                  <td>วันฉัตรมงคล</td>
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>6</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
            <td>
              <table class="inside-table-month">
                <thead>
                  <tr>
                    <th>7</th>
                  </tr>
                </thead>
                <tbody>
                  <!-- <td></td> -->
                </tbody>
              </table>
            </td>
          </tr>
        </tbody>
      </table>
            
    <a class="btn-floating btn-large waves-effect waves-light red"><i class="material-icons">add</i></a>

    <!--Import jQuery before materialize.js-->
    <script type="text/javascript" src="https://code.jquery.com/jquery-2.1.1.min.js"></script>
    <script type="text/javascript" src="js/materialize.min.js"></script>
    <script type="text/javascript" src="js/editjs.js"></script>


  </body>
</html>